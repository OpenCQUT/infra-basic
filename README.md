...
`infra-basic` 是一条面向 **LLM 推理基础设施** 的学习路线。它从 PyTorch / Transformers 推理开始，逐步进入 Operator / Kernel、KV Cache、serving、benchmark、vLLM、SGLang、论文阅读和开源贡献。
...
4. 写一个极简 LLM HTTP serving demo，并完成基础压测；
5. 理解 vLLM 的 API server、engine、scheduler、KV cache manager、model runner、attention backend；
6. 理解 SGLang 的 runtime、RadixAttention、structured output、prefix cache、offline engine；
7. 读懂 LLM serving 领域的基础、经典和较新的论文；
8. 以文档、测试、benchmark、bug reproduction、小功能为入口参与 vLLM / SGLang 开源贡献。
...
不要一开始直接啃 CUDA kernel、分布式部署或大规模生产架构。先把 PyTorch tensor、operator/kernel、单请求生成、batch、prefill/decode、KV cache、continuous batching、prefix cache 这些主路径建立起来。
